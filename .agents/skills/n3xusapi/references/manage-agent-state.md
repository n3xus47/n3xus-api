# Manage Agent State — n3xusAPI Endpoint Reference

Generated endpoint reference for the `manage-agent-state` rows of the `n3xusapi` skill router. Bundle version: 00b181aa4f09. This file is always managed — it is refreshed with the bundle even when `../SKILL.md` has been customized.
...
Safety:
- Local n3xusAPI: `debitMicrousd` is always 0; use balance only to confirm you hit the local instance, not for top-ups.
...
List every n3xusAPI capability with its live status, or pass capability=<slug> to read one capability's full live contract.
...
Send a bug report, idea, or praise to the n3xusAPI team. Free, any active key.
