# Send Email — n3xusAPI Endpoint Reference

Generated endpoint reference for the `send-email` rows of the `n3xusapi` skill router. Bundle version: 00b181aa4f09. This file is always managed — it is refreshed with the bundle even when `../SKILL.md` has been customized.
...
- Emails from the standard n3xusAPI sending domain include a mandatory linked 'Sent via n3xusAPI' footer. Verified customer-owned domains do not.
...
```json
{
  ...
  "properties": {
    "emailIdentityId": {
      "type": "string",
      "description": "Optional n3xusAPI email identity id. Omit to use the workspace default."
    },
    "confirmInboxCharge": {
      "type": "boolean",
      "default": false,
...
```json
{
  ...
  "properties": {
    ...
    "domain": {
      "type": "string",
      "description": "Optional verified custom domain from GET /v1/email/domains. Omit to use the standard n3xusAPI sending domain."
    },
    "dryRun": {
      "type": "boolean",
      "default": false,
...
- The full address is returned once. Replaying the original request returns it masked, because n3xusAPI does not keep the address after answering.
...
```json
{
  ...
  "properties": {
    ...
    "company": {
      "type": "string",
      "maxLength": 200,
      "description": "Company name such as Stripe. Use it when the domain is unknown; n3xusAPI resolves it to a domain. It is optional and ignored when domain is supplied."
    },
    "maxCostUsd": {
      "type": "string",
      "pattern": "^\\d+(\\.\\d{1,6})?$",
