# Scraping — n3xusAPI Workflow Guide

Managed workflow guide for the `scraping` rows of the `n3xusapi` skill router. Bundle version: 00b181aa4f09. This file is always managed — it is refreshed with the bundle even when `../SKILL.md` has been customized.
...
- Do not send a GitHub token or OAuth credential; n3xusAPI uses unauthenticated public GitHub REST (rate limits apply).
...
- When the origin publishes JSON-LD, the page also includes `structuredData` (flattened `@graph` objects). The useful ingest field is `recipe`: JSON-LD if present, otherwise HTML recipe markup, otherwise **markdown** (Ingredients headings + lists, or consecutive quantified lines like `200 g soku` / `1 teaspoon salt`). `source` is `jsonld`, `html`, or `markdown`. Most recipe blogs have no JSON-LD; use `recipe`, not a full-page markdown dump.
- `POST /v1/scrape/extract` fills the requested schema from `recipe` / JSON-LD first. The local model runs only when those are missing. “Extract the recipe” is a valid extraction instruction, not a trivia question.
...
- `$N3XUS_API_BASE_URL/docs` is the interactive OpenAPI UI.
- `$N3XUS_API_BASE_URL/openapi.json` is the machine-readable contract when exposed by your build.
- Repo `README.md` and `docs/deepapi-parity-audit.md` document local support levels vs hosted DeepAPI.
