...
from app.youtube import YouTubeError, metadata as youtube_metadata, thumbnail as youtube_thumbnail, transcript as youtube_transcript
...
async def youtube_response(route: str, payload: dict, raw: Request, action):
    ...
    return persist(route, raw, envelope(route=route, capability="scrape.youtube", output=await action()), False)
...
async def youtube_resources(resource: str, payload: dict, raw: Request):
    ...
    if resource == "search" and isinstance(payload.get("query"), str):
        return await youtube_response(raw.url.path, payload, raw, lambda: youtube_metadata(f"ytsearch{max_items}:{payload['query']}", max_items=max_items))
    if resource in {"channel", "shorts"} and isinstance(payload.get("channels"), list):
        return await youtube_response(raw.url.path, payload, raw, lambda: youtube_metadata(f"https://www.youtube.com/@{payload['channels'][0]}/videos", resource == "shorts", max_items))
